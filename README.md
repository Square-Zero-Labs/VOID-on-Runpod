...
- Grid-bounds validation for VLM outputs. This repo explicitly teaches Gemini the valid row and column ranges for each video's computed grid, clamps any out-of-bounds trajectory or localization coordinates during parsing, and clips trajectory boxes to the visible grid so off-frame predictions do not silently erase affected-mask coverage.
